...
def find_structure_in_radius(
    ...
    region_size = get_structure_region_size(
        structure,
        version,
    )

    # Structure regions are measured in chunks.
    # Convert our block search radius into a safe region range.
    ...
    for region_x in range(-region_radius, region_radius + 1):
        for region_z in range(-region_radius, region_radius + 1):
            ...
            candidate = get_structure_position(
                structure,
                seed,
                region_x,
                region_z,
